...
export default function QuestionnaireFeedback({ feedback, answers, questionnaire }: QuestionnaireFeedbackProps) {
  ...
  const normalizedFeedback = () => {
    ...
    // If feedback only has feedback property
              ...
              return (
                <div key={index} className={`p-4 rounded-md border ${getFeedbackQualityClass(item)}`}>
                  ...
                  <div className="space-y-3">
                    ...
                    {item.improvement && (
                      <div className="mt-2">
                        <p className="text-xs font-medium text-gray-600 mb-1">Improvement Suggestion:</p>
                        <div className="p-2 bg-yellow-50 rounded-md text-yellow-800 text-sm">{item.improvement}</div>
                      </div>
                    )}
                  </div>
                </div>
